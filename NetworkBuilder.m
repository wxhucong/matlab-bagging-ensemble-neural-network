function [weights, baises] = NetworkBuilder(nodeLayers)
    weights = {};
    baises = {};
    ...
    elseif(length(nodeLayers)==2)
        ...
        weights = normrnd(0,1,[nodeLayers(2) nodeLayers(1)]) ./ sqrt(nodeLayers(1));
        baises = normrnd(0,1,[nodeLayers(2) 1]);
    else
        for i = 1:(length(nodeLayers)-1)
            W = normrnd(0,1,[nodeLayers(i+1) nodeLayers(i)]) ./ sqrt(nodeLayers(i));
